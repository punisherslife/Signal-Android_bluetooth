...
def insert_before_outer_class_end(rel: str, snippet: str, guard: str) -> None:
    """Insert a Java member immediately before the outer class' final brace."""
    path = ROOT / rel
    text = path.read_text(encoding="utf-8")
    if guard in text:
        raise RuntimeError(f"{rel}: patch already present: {guard!r}")

    closing = text.rfind("\n}")
    if closing == -1:
        raise RuntimeError(f"{rel}: could not locate outer class closing brace")
